...
for row in qres:
    print("The result of the number of beds in Glasgow Royal Infirmary is: %s" % row)
...
qres = g.query(
    """SELECT ?c
    WHERE {
    <http://dbpedia.org/resource/Royal_Infirmary_of_Edinburgh>  <http://dbpedia.org/ontology/country> ?c .
    }""")

for row in qres:
    print("The Royal Infirmary of Edinburgh is located in %s" % row)
